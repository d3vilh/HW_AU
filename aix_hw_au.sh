...
printf "SITE |HOSTNAME |HW TYPE |SYSTEM MODEL |SERIAL |NGSCORE |DBCORE |ORACLE DB |ORACLE CLI |UP VERSION |JAVA VERSION |FIRMWARE |AIX OS LEVEL |BLU MODEL |BLU SERIAL |NSR LICENSE EXP |NETWORKER VERSION |FCSWA MODEL |FCSWA SN |FCSWA FW LEVEL |FCSWB MODEL |FCSWA SN |FCSWB FW LEVEL |EMC MODEL |EMC SERIAL |EMC FLARE |V7k MODEL |V7k TYPE |V7k ENCLOSURE SN |v7K FW |V7k failed HDDs |V7k CONSOLE |V7k2F MODEL |V7k2F TYPE |V7k2F ENCLOSURE SN |v7K2F FW |V7k2F failed SSDs |V7k2F CONSOLE |CLUST IP |NODE IP |HMC IP |LPAR INFO |AUTO RESTART |CPU CLOCK |NUM OF PHY CPU |CPU SMT MODE |NUM OF LOG CPU |RAM SIZE |GOOD RAM SIZE |NUM OF RAM MODULES |SIZE OF RAM MODULES(MB) |PAGE SIZE |COUNT ERRPT | UNIQ ERRPT |UPTIME |\n";
...
for host in $(grep -iE $host_match /etc/hosts|grep -viE "$aix_ex_tmplt"|awk {'print$2'}|sort|uniq | grep -E 'a|b');
	...
	printf \"\$hw_type |\$sys_model |\$serial |\$ngscore |\$dbcore_ver |\$orad_ver |\$orac_ver |\$up_version |\$java_version |\$fw |\$os_level |\$blu |\$blu_sn |\$nsr_license |\$nsr_version |\$fcswa_model |\$fcswa_sn |\$fcswa_fw |\$fcswb_model |\$fcswb_sn |\$fcswb_fw |\$emc |\$v7000_model |\$v7000_enclosure_type |\$v7000_sn |\$v7000_fw |\$v7000_fhdd |\$v7000_ip |\$v7000f_model |\$v7000f_enclosure_type |\$v7000f_sn |\$v7000f_fw |\$v7000f_fhdd |\$v7000f_ip |\$clust_ip |\$ip_addr |\$hmc_ip|\$lpar |\$autorest |\$cpu_speed |\$num_of_cpu |\$cpu_smt_mode |\$num_cpu_cores |\$ram_size |\$good_ram_size |\$num_of_ram_modules |\$size_of_ram_modules |\$page_size |\$errpt |\$uniq_errpt |\$uptm |\";";
	printf "\n";
done;
#thats all folks!