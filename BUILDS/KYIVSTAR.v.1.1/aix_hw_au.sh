...
# v.06.2.2 small fix. #v.06.2.1 Java and UP VER added #v.06.1 DBCORE VER added #v.06.0 P9 support added. #v.05.3 Added FW level Cisco MDS #v.05.2 30.08.2019 mass "Unknown format" fix. #v.05.1 21.08.2019 nsradmin and v7k get bugs fixed for NGSCORE 7.x, Oracle version added. # v0.1 15.11.2015, v.02 14.03.2018 NSR license ext date added, v.03 27.07.2018 WA for stderr and for flash storage3, v.04 21.03.2019 NXOS FCSW & Networker version added
if [ ! -n "$1" ]; then 
...
printf "SITE |HOSTNAME |HW TYPE |SYSTEM MODEL |SERIAL |NGSCORE |DBCORE |ORACLE DB |ORACLE CLI |UP VERSION |JAVA VERSION |FIRMWARE |AIX OS LEVEL |BLU MODEL |BLU SERIAL |NSR LICENSE EXP |NETWORKER VERSION |FCSWA MODEL |FCSWA SN |FCSWA FW LEVEL |FCSWB MODEL |FCSWA SN |FCSWB FW LEVEL |EMC MODEL |EMC SERIAL |EMC FLARE |V7k MODEL |V7k TYPE |V7k ENCLOSURE SN |v7K FW |V7k failed HDDs |V7k CONSOLE |V7k2F MODEL |V7k2F TYPE |V7k2F ENCLOSURE SN |v7K2F FW |V7k2F failed SSDs |V7k2F CONSOLE |CLUST IP |NODE IP |HMC IP |LPAR INFO |AUTO RESTART |CPU CLOCK |NUM OF CPU |RAM SIZE |GOOD RAM SIZE |NUM OF RAM MODULES |SIZE OF RAM MODULES(MB) |PAGE SIZE |COUNT ERRPT | UNIQ ERRPT |UPTIME |\n";
for host in $(grep -iE $delat_krasivo /etc/hosts|grep -viE '^ *#|localhost|farm|blu|acmi|admin|emc|v7000|om|hmc|fcs|emc|tape|asmi|zbx|hsbu|mau|rctu'|awk {'print$2'}|sort|uniq | grep -E 'a|b');
...
naviseccli -user root -password comverse -scope 0 -h emc1 getagent > /tmp/navi_agent.txt 2>/dev/null 
...
up_version=\`mshell secadmin/passw0rd 'version' 2>/dev/null| grep AIX | awk '{print \$8}'\`
...
if [[ -z \$dbcore_ver ]]; then dbcore_ver=\`printf \"Check other node\"\`; fi  
...
printf \"\$hw_type |\$sys_model |\$serial |\$ngscore |\$dbcore_ver |\$orad_ver |\$orac_ver |\$up_version |\$java_version |\$fw |\$os_level |\$blu |\$blu_sn |\$nsr_license |\$nsr_version |\$fcswa_model |\$fcswa_sn |\$fcswa_fw |\$fcswb_model |\$fcswb_sn |\$fcswb_fw |\$emc |\$v7000_model |\$v7000_enclosure_type |\$v7000_sn |\$v7000_fw |\$v7000_fhdd |\$v7000_ip |\$v7000f_model |\$v7000f_enclosure_type |\$v7000f_sn |\$v7000f_fw |\$v7000f_fhdd |\$v7000f_ip |\$clust_ip |\$ip_addr |\$hmc_ip|\$lpar |\$autorest |\$cpu_speed |\$num_of_cpu |\$ram_size |\$good_ram_size |\$num_of_ram_modules |\$size_of_ram_modules |\$page_size |\$errpt |\$uniq_errpt |\$uptm |\";";printf "\n";done;
#thats all folks!